...
{name:'Bases Profesionales',brand:'Dolce Bella',price:6.00,category:'Rostro',image:'DOLCE_BELLA_Bases_Profesionales.png'},
...
{name:'Blush Horneado',brand:'Dolce Bella',price:5.80,category:'Rostro',image:'DOLCE_BELLA_Blush_Horneado.png'},
...
{name:'Corrector Líquido',brand:'Lancôme',price:5.50,category:'Rostro',image:'LANCOME_Corrector_Liquido.png'}
];
...
function renderProducts(){
  ...
  document.querySelector('#products').innerHTML=filtered.map(p=>{const qty=cart[p.name]||0;return `<article class="card"><img src="${p.image}" alt="${p.name} — ${p.brand}" loading="lazy" onerror="this.onerror=null;this.alt='Imagen no disponible: ${p.name}';this.style.opacity='.35'"><div class="card-content"><span class="category-label">${p.brand} · ${p.category}</span><h3>${p.name}</h3><div class="price">${money(p.price)}</div><div class="product-actions"><button class="add-button" type="button" onclick="addToCart('${p.name.replaceAll("'","\\'")}')">➕ Añadir al carrito</button>${qty?`<span class="in-cart">${qty} en carrito</span>`:''}</div></div></article>`}).join('');
...
function renderCart(){const lines=cartLines();const units=lines.reduce((s,x)=>s+x.qty,0);const total=lines.reduce((s,x)=>s+(x.product.price===null?0:x.product.price*x.qty),0);const hasConsult=lines.some(x=>x.product.price===null);document.querySelector('#cartCount').textContent=units;document.querySelector('#summaryUnits').textContent=units;document.querySelector('#cartTotal').textContent=money(total);document.querySelector('#consultNotice').hidden=!hasConsult;document.querySelector('#cartEmpty').hidden=lines.length>0;document.querySelector('#clearCart').disabled=lines.length===0;document.querySelector('#checkout').disabled=lines.length===0;document.querySelector('#cartItems').innerHTML=lines.map(({product:p,qty})=>`<div class="cart-item"><img src="${p.image}" alt="${p.name}"><div class="cart-item-info"><h3>${p.name}</h3><small>${p.brand}</small><strong>${money(p.price*qty)}</strong><div class="qty-controls"><button type="button" onclick="decrease('${p.name.replaceAll("'","\\'")}')">−</button><span>${qty}</span><button type="button" onclick="addToCart('${p.name.replaceAll("'","\\'")}')">+</button><button class="remove" type="button" onclick="removeFromCart('${p.name.replaceAll("'","\\'")}')">Eliminar</button></div></div></div>`).join('');}
...
function checkout(){const lines=cartLines();if(!lines.length)return;const total=lines.reduce((s,x)=>s+x.product.price*x.qty,0);const detail=lines.map(x=>`${x.qty} x ${x.product.name} — ${money(x.product.price*x.qty)}`).join('\n');const text=`Hola, quiero realizar este pedido en ${config.businessName}:\n\n${detail}\n\nTotal de productos: ${lines.reduce((s,x)=>s+x.qty,0)}\nTotal a pagar: ${money(total)}\n\n¿Me confirman disponibilidad y cómo continuar con el pago?`;window.open(`https://wa.me/${config.whatsapp}?text=${encodeURIComponent(text)}`,'_blank','noopener');}
function setCategory(c){activeCategory=c;renderCategories();renderProducts();}
function init(){document.title=config.businessName;document.querySelector('#mainWhatsapp').href=wa('este catálogo');document.querySelector('#footerWhatsapp').href=wa('este catálogo');if(typeof QRCode!=='undefined')new QRCode(document.querySelector('#qrcode'),{text:'https://ventas-online.github.io/Catalago/',width:140,height:140});renderCategories();renderProducts();renderCart();document.querySelector('#search').addEventListener('input',renderProducts);document.querySelector('#openCart').addEventListener('click',openCart);document.querySelector('#closeCart').addEventListener('click',closeCart);document.querySelector('#cartOverlay').addEventListener('click',closeCart);document.querySelector('#checkout').addEventListener('click',checkout);document.querySelector('#clearCart').addEventListener('click',clearCart);}
init();
